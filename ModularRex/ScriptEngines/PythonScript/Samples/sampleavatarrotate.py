...
class RotTowardsMe(rxactor.Actor):
    ...
    def EventTouch(self,vAvatar):
        avatarforward = vAvatar.llRot2Fwd(vAvatar.llGetRot())
        avatartoward = self.llVecNorm(self.llGetPos() - vAvatar.llGetPos())
        rotresult = self.llRotBetween(avatarforward,avatartoward)
        vAvatar.SetRelativeRot(rotresult)
