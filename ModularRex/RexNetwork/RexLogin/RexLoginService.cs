...
namespace ModularRex.RexNetwork.RexLogin
{
    ...
    {
        LoginResponse Login(string account, string sessionHash, string startLocation, IPEndPoint clientIP);
    }
    ...
    public class RexLoginService : LLLoginService, IRexLoginService
    {
        ...
        public LoginResponse Login(string account, string sessionHash, string startLocation, IPEndPoint clientIP)
        {
            bool success = false;
            ...
            try
            {
                ...
                if (m_PresenceService != null)
                {
                    ...
                    {
                        home = m_GridService.GetRegionByUUID(useraccount.ScopeID, presence.HomeRegionID);
                    }
                }
                ...
                GridRegion destination = FindDestination(useraccount, presence, session, startLocation, out gatekeeper, out where, out position, out lookAt);
                if (destination == null)
                {
                ...
                AgentCircuitData aCircuit = LaunchAgentAtGrid(gatekeeper, destination, useraccount, avatar, session, secureSession, position, where, out where, out reason);
